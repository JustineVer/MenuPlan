(function() {
    ...
    AutoFillController.$inject = ['$scope','$rootScope','$q', 'typeService', '$log', 'sharedData', 'dataService'];

    function AutoFillController($scope, $rootScope, $q, typeService, $log, sharedData, dataService) {
        var vm = this;
...
//                    vm.allMeals = meals;
            ...
            function generatePlan () {
                if (!sharedData.mealsAvailableToAutoFill)
                {
                    sharedData.mealsAvailableToAutoFill = sharedData.allMeals;
                }
                var autoFilledMeals = [];
                if (!mealId){
                ...
                {
                    //remove selected meal from autoFilledMeals array
                    autoFilledMeals = sharedData.autoFilledMeals.filter(function( obj ) {
                        return obj.id !== mealId.id;
                      });
                    //and meals available to autofill array
                    sharedData.mealsAvailableToAutoFill = sharedData.mealsAvailableToAutoFill.filter(function( obj ) {
                    return obj.id !== mealId.id;
                    });
                ...
                function generate (mType, NoMeatTypes, typeOption){
                    ...
                    var filteredMeals = sharedData.mealsAvailableToAutoFill.reduce(function (filteredMeals, meal) {
                        if (typeOption == 'meat' ){
                            if (meal.meat_type_id !== null && (new RegExp("\\b"+mType+"\\b").test(meal.meat_type_id))) {
                        ...
                        if (typeOption == 'meal' ){
                            if (meal.meal_type_id !== null && (new RegExp("\\b"+mType+"\\b").test(meal.meal_type_id))) {
                            return filteredMeals.concat(meal);
                            } else {
                                return filteredMeals;
                            }
                    ...
                    for (x = 0; x < NoMeatTypes; x++) {
                        if (filteredMeals.length == 0){
                            var errorMessage = 'No more meals to select from for '+typeOption+': ';
                            if (typeOption == 'meat') {
                                errorMessage += findMeatTypeDesc(mType)+' ('+NoMeatTypes+')';
                            } else if (typeOption == 'meal') {
                                errorMessage += findMealTypeDesc(mType)+' ('+NoMeatTypes+')';
                            }
